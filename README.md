# 👋 Hi, I'm Abran

🚀 Full-stack Java Developer | Mobile App Developer (React Native)  
📍 Based in Berlin, Germany  

---

## 🧠 About Me

- 👨‍💻 3.5+ years experience as a Full-Stack Java Developer  
- 📱 Currently building mobile apps using **React Native (Expo) + Supabase**  
- 🌱 Learning **Kotlin & modern backend architectures**  
- 🎯 Focused on building scalable, user-friendly applications  

---

## 🚀 Current Project

### 📚 Ezhilagam
A Tamil literature learning app built with React Native and Supabase.

- 📖 Explore classical Tamil works (Thirukkural, Kamba Ramayanam, Bharathiyar)
- 🔐 Authentication with Supabase
- 🎨 Clean cultural UI (Purple + Gold theme)

👉 *More features coming soon: audio, bookmarks, progress tracking*

---

## 🛠️ Tech Stack

### 💻 Languages
- Java
- JavaScript / TypeScript
- Kotlin (learning)

### ⚙️ Frameworks & Tools
- React Native (Expo)
- Spring Boot
- Supabase
- Node.js

### 🎨 Frontend
- React
- HTML / CSS

---

## 👨‍💻 Profile Views

![](https://komarev.com/ghpvc/?username=Abran7411&color=blueviolet)

## 📊 GitHub Stats

<div align="center">
  <img height="170" src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=Abran7411&theme=dracula"/>
  <img height="170" src="https://streak-stats.demolab.com?user=Abran7411&theme=dracula"/>
</div>

---

## 📫 Contact Me

- 📧 Email: contact@abran.work  
- 💼 LinkedIn: https://www.linkedin.com/in/abran-p  

---

## ⚡ Fun Fact

I enjoy building apps that combine **culture + technology**, like bringing Tamil literature into modern mobile experiences.

---

## 🐍 Contributions

![Snake animation](https://github.com/Abran7411/Abran7411/blob/output/github-contribution-grid-snake.svg)
